...
//
//  Created by SDMobile on 4/19/16.
...
class SquareShapeViewFactory: ShapeViewFactory {
  ...
  func makeShapeViewsForShapes(shapes: (Shape, Shape)) -> (ShapeView, ShapeView) {
    ...
    let shapeView2 = SquareShapeView(frame: CGRect(x: 0, y: 0, width: squareShape2.sideLength * size.width, height: squareShape2.sideLength * size.width))
